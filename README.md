...
当用户发送包含图片的消息时，系统会自动处理：

1. **下载图片** - 从消息中提取图片URL并下载到临时文件
2. **上传图片** - 将图片上传到 DeepSeek 获取 `file_id`
3. **清理临时文件** - 上传成功后自动删除本地临时文件
4. **AI分析** - 将图片ID传递给 DeepSeek 进行内容分析
...
用户: @小鹿 这张图片里是什么？
(附带一张风景图片)
AI: 这是一张美丽的风景照片呢～有山有水，天空蓝蓝的，杂鱼哥哥/姐姐是在旅游吗？
```
...
│   ├── 📄 __init__.py               模块导出
│   ├── 📄 deepseek_client.py      DeepSeek API 客户端（含图片上传）
│   ├── 📄 ai_manager.py           对话管理器（上下文记忆 + 多模态）
